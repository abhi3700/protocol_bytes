...
# Exercise 2a: Transaction Replacement with the Default Price Bump

Background

Ethereum transactions are identified for replacement by the combination of:

sender + nonce

If Alice submits two transactions with the same nonce, the second one is treated as a replacement candidate for the first.

However, Reth does not allow an arbitrary replacement. The new transaction must increase the relevant fee fields enough to satisfy the transaction pool’s configured replacement threshold.

For EIP-1559 transactions, the important fee fields are:
...
Reth’s default transaction replacement threshold is approximately:

10%

So, if the existing transaction has:

```
max_fee_per_gas = 1 gwei
```

then a transaction with only a tiny increase such as:

```
1 gwei + 1 wei
```

is still considered too cheap.

Reth rejects it with:

```
replacement transaction underpriced
```

This exercise demonstrates that default behavior.

## Problem

On a single Reth node:

1. Create Alice with sufficient balance.
2. Send an EIP-1559 transaction with nonce = 0.
3. Verify the transaction enters Alice’s pending pool.
4. Create another transaction from Alice with the same nonce = 0.
5. Increase the fees only slightly.
6. Submit the replacement and verify that Reth rejects it with:

replacement transaction underpriced

1. Create another replacement whose fee satisfies the default replacement threshold.
2. Submit it.
3. Verify that Alice still has exactly one pending transaction.
4. Verify that the pending transaction is now the replacement transaction rather than the original.
...
and therefore treats tx2 as a replacement candidate.

But before replacing the transaction, the pool performs its replacement-price check:

existing tx
    │
    ▼
required replacement bump
    │
    ▼
new tx fee sufficient?
   / \
 no   yes
 │     │
 ▼     ▼
...
Because the tiny fee increase does not satisfy the default threshold:
...
The original transaction remains in the pool.
