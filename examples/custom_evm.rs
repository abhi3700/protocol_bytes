...
#![doc = include_str!("../docs/custom_evm.md")]
...
pub fn prague_custom() -> &'static Precompiles {
	...
	INSTANCE.get_or_init(|| {
		...
		// Custom precompile.
		let precompile =
			Precompile::new(PrecompileId::custom("custom"), STATEFUL_SYS_ADDR, |_, _| {
...
fn genesis_with_stateful_system_contract() -> Genesis {
	let runtime_code: Bytes = hex!("6000546001018060005560005260206000f3").to_vec().into();

	let mut genesis = Genesis { base_fee_per_gas: Some(1_000_000_001), ..Default::default() };
	genesis.alloc.insert(
		STATEFUL_SYS_ADDR,
		GenesisAccount {
			nonce: Some(1),
			balance: U256::ZERO,
			code: Some(runtime_code),
			storage: None,
			private_key: None,
		},
	);
...
async fn main() -> eyre::Result<()> {
	...
	let spec = ChainSpec::builder()
		.chain(Chain::mainnet())
		.genesis(genesis_with_stateful_system_contract())
		.london_activated()
		.paris_activated()
		.shanghai_activated()
		.cancun_activated()
